...
このプロジェクトは、Python (uv workspace) + Kotlin Multiplatform + Rust runtime のモノレポです。
...
├── egopulse/              # Rust runtime foundation（Cargo）
│
├── docs/                  # プロジェクトドキュメント
...
| **egopulse/**  | Rust agent runtime foundation | Rust, Tokio, reqwest, Clap                                     | Cargo / GitHub Actions    |
...
```bash
# OpenAI-compatible endpoint に単発問い合わせ
...
- **ci-egopulse.yml**: `egopulse/` と Rust workspace の変更時

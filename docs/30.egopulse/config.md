...
10. [CLI ランタイムコマンド](#10-cli-ランタイムコマンド)
11. [再起動要否と秘匿フィールド](#11-再起動要否と秘匿フィールド)
12. [インターフェース間比較マトリクス](#12-インターフェース間比較マトリクス)
...
| `bot_token` | `string \| SecretRef` | 条件付き | なし | Discord Bot トークン。有効時は必須。SecretRef 使用可能。環境変数 `EGOPULSE_DISCORD_BOT_TOKEN` でも指定可能。秘匿フィールド |
| `provider` | `string \| null` | 任意 | `null` | このチャネル専用のプロバイダーオーバーライド |
| `model` | `string \| null` | 任意 | `null` | このチャネル専用のモデルオーバーライド |
...
| `bot_token` | `string \| SecretRef` | 条件付き | なし | Telegram Bot トークン。有効時は必須。SecretRef 使用可能。環境変数 `EGOPULSE_TELEGRAM_BOT_TOKEN` でも指定可能。秘匿フィールド |
...
2. **Layer 2: プロセス環境変数オーバーライド** — `EGOPULSE_WEB_AUTH_TOKEN` 等が Layer 1 の結果を上書き
...
| Web 認証トークン | `EGOPULSE_WEB_AUTH_TOKEN` |
| Discord Bot トークン | `EGOPULSE_DISCORD_BOT_TOKEN` |
| Telegram Bot トークン | `EGOPULSE_TELEGRAM_BOT_TOKEN` |

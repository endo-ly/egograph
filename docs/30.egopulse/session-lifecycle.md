...
tool call は text として埋め込み、image は binary を再注入せずプレースホルダ化する。

要約入力が長すぎる場合は 20,000 文字で打ち切り、末尾に `... (truncated)` を付ける。
...
- `compact_keep_recent < max_session_messages`

最後の条件は、summary block が追加される構造上、等値を許すと compaction が実質的に効きにくくなるため。
